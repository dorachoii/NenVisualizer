    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float GetWave(float2 uv)
            {
                ...
                // 거리에 비례하는 높이 계산 (중심에서 높고, 가장자리로 갈수록 낮아짐)
                float heightMultiplier = 1 - pow(radialDistance / _MaxRadius, _HeightFalloff);
                heightMultiplier = max(0, heightMultiplier); // 음수 방지
